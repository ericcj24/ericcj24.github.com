---
layout: post
title: "OAuth2 Authorization Use Python"
tags: [API]
...
# to this point, you finished first step, obtain authorization code
...
r = requests.post(v2,data = v2)
r.status_code # should be 200 if everything is right
r_json = r.json()
# now you could see response in json format, with access_token in it
{% endhighlight %}

Just remember that first stage use HTTP method GET, second stage use HTTP method POST.(that means you have to pass your encoded v2 to data, instead of params!!!)
